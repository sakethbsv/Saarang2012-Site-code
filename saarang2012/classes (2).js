...
function Site () {
	...
	// this.addCustomBox = function ( title, content ) {
...
function AJAX () {
	...
	this.prepareAndDispatch = function ( box ) {
		...
		this.error = box.processAJAXError;
		this.sendRequest();
	};
...
function Box ( parent, index ) {
	...
	this.load = function ( parent, index ) {
		...
		/*if(this.id=="box_home_proshow"){$('#proshowbg').show(50);}*/
	...
	this.addSlideshowBox = function () {
		
		var id = "slideshowbox";
		var jQSelector = $("#"+id);
		var numPics = this.config.Slideshow.length;
		var html = "";
		for( var i = 0; i < numPics; ++i ) {
			var src = this.config.Slideshow[i];
			html = html + "<img src=" + src + " width=\"" + "222" + "\" height=\"" + "195" + "\">";
		}
		
		id = this.id + "_slideshow";
		
		innerHtml = "<div id=\"" + id + "\" class=\"classOuterBoxSlideshow\" >" + html + "</div>";
		$(jQSelector).html("");
	...
	this.addGalleryBox = function () {
		...
		this.jQSelector.addClass("defaultClassGalleryBox");
	...
	this.addInnerBox = function ( index, title, content ) {
		...
		if( index == -1)
			...
			innerHtml = "<div id=\"" + id + "_title" + "\" class=\"classInnerBoxGrid\" ><p>" + gridText + "</p></div>";
		else if( index == 0)
		...
		if(id == box_sponsors_1){innerHtml = "<div id=\"" + id + "_title" + "\" class=\"classInnerBoxTitle\" ><p>" + title + "</p></div>" +
							"<div id=\"" + id + "_content" + "\" class=\"classInnerBoxContent\">" + 
							"<div id=\"" + id + "_scroll" + "\" class=\"classInnerBoxScroll1\" ><div class=\"customScrollBox\"><div class=\"container\">" +
							"<div class=\"content\">" + content + "</div></div>" + 
							"<div class=\"dragger_container\"><div class=\"dragger\"></div></div></div></div></div>";
			
		}else{
			innerHtml = "<div id=\"" + id + "_title" + "\" class=\"classInnerBoxTitle\" ><p>" + title + "</p></div>" +
							"<div id=\"" + id + "_content" + "\" class=\"classInnerBoxContent\">" + 
	...
	this.attachHandlers = function () {
		...
		if( this.type == "box_zoom" ) {
			...
			//this.jQSelector.bind( "click", function() {$('#proshowbg').show(50);} );	
			$('#box_pro_close').bind('click', function() {$('#proshowbg').hide(50);});
			$('#box_pro_dsp_tab_close,#box_pro_vs_tab_close,#box_pro_rockshow_tab_close,#box_pro_classical_tab_close').bind('click', function() {$('#proshowbg').css('background-image', 'url(images/The-best-top-desktop-lightning-wallpapers-lightning-wallpaper-4.jpg)');});
						$('#box_pro_dsp').bind('click', function() {$('#proshowbg').css('background-image', 'url(images/DSP.jpg)');});
			$('#box_pro_vs').bind('click', function() {$('#proshowbg').css('background-image', 'url(images/vs.jpg)');});
			$('#box_pro_rockshow').bind('click', function() {$('#proshowbg').css('background-image', 'url(images/mainbg1.jpg)');});
			$('#box_pro_classical').bind('click', function() {$('#proshowbg').css('background-image', 'url(images/dsp2.jpg)');});
			//$('#box_pro_dsp').bind('click', function() {$("#proshowbg").css('background-image', 'url(images/DSP.jpg)');$('#proshowbg').css('background-image', 'url(images/DSP-Poster-1-(Web).jpg)');});
			//box_pro_dsp_tab_close
	...
	this.zoomIn = function ( event ) {
		...
		thisBox.jQSelector.addClass("defaultClassZoomedInBox");
	};
	...
	this.zoomInAnimate = function ( event ) {
		var slideshowid = "#" + this.id + "_Slideshow_container";
		var crap2 = "#" + this.id + "_Slideshow";
		//alert(slideshowid);
	
		
		
		$(slideshowid).css( "display", "block" );
		$(crap2).css( "display", "block" );
	...
	this.processAJAXResponse = function ( data, textStatus, jqXHR ) {
		...
		this.animateAndHideTabs( );
		
		if( this.config.defaultTab ) {
			this.activeTabIndex = this.config.defaultTab;
			this.activeTabs = this.tabs[ this.activeTabIndex ].config.activeTabs;			
		} else {
			this.activeTabs = [ 0 ];
		}
		
		( function (thisBox, delay) {
			setTimeout(function() 	{
				thisBox.animateAndShowTabs();
				thisBox.hideInnerBox( 1 );
	...
	this.processAJAXError = function ( jqXHR, textStatus, errorThrown ) {
		
		var loadingBox = this.innerBoxjQSelectors[1];
		
		loadingBox.find('div:first').html("<center><br /><br /><br /><br /><br /><br /><p>This is taking longer than expected.</p><br /><br /><br /><br /><p>Please confirm that you are connected to the internet and try again by closing and opening this tile. <p></center>");
	...
	this.zoomOut = function () {
		...
		this.jQSelector.removeClass("defaultClassZoomedInBox");
	};
	...
	this.switchTabAnimate = function ( event ) {
		...
		var newActiveTabIndex = thisTab.index + 1;
		var newActiveTabs = thisTab.activeTabs;
		
		if( equalArrays( thisBox.activeTabs, newActiveTabs ) ) {
			
			thisBox.hideInnerBox(thisBox.activeTabIndex + 1);
			var tab = thisBox.tabs[ thisBox.activeTabIndex ];
			
			var highlightOutDuration =  config.defaults.tab.highlightOutDuration;
			tab.jQSelector.css( { "background-color" : tab.parent.backgroundColor  } ).animate( { "background-color" : tab.backgroundColor }, { duration : highlightOutDuration, queue : false } );
			tab.attachHandlers();
			
			thisBox.setActiveTab( event );
			
			thisBox.deactiveActiveTab();
			var highlightInDuration =  config.defaults.tab.highlightInDuration;
			thisTab.jQSelector.css( { "background-color" : thisTab.parent.backgroundColor } );
			//thisTab.jQSelector.css( { "background-color" : thisTab.backgroundColor } ).animate( { "background-color" : thisTab.parent.backgroundColor }, { duration : highlightInDuration, queue : false } );
...
/*
function CustomBox ( parent, index ) {
